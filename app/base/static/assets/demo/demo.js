...
demo = {
  ...
  showNewTweetNotification: function(from, align, influencer_name, token_name, token_calls, within_last_interval) {
    color = 2
    const callsNum = token_calls

    $.notify({
      icon: "tim-icons icon-bell-55",
      message: `<b>${ influencer_name }</b> - just made a tweet about <b>${ token_name }</b> (${ callsNum } in ${ within_last_interval.toLowerCase() }.<br>${ callsNum < 5 ? '🔥' : callsNum < 15 ? '🔥🔥' : '🔥🔥🔥'}).`
